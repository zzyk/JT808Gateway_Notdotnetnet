...
namespace JT808Server.Application.Services
{
    ...
    public class TCPServer : IHostedService
    {
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            WinFromDisplay.ShowSysMsg("",$"JT808 TCP Server start at {IPAddress.Any}:{ConfigurationMonitor.CurrentValue.TcpPort}.");
            Task.Run(async () =>
            {
                    ...
                    try
                    {
                        ...
                        await Task.Factory.StartNew(async (state) =>
                        {
                            ...
                            if (Logger.IsEnabled(LogLevel.Information))
                            {
                                ...
                                WinFromDisplay.ShowSysMsg("",$"[Connected]:{session.Client.RemoteEndPoint}");
                            }
                            var pipe = new Pipe();
...
# if DEBUG
        ...
        private void Processor(in IJT808Session session, in JT808HeaderPackage package)
        {
            try
            {
                ...
                WinFromDisplay.ShowSysMsg(package.Header.TerminalPhoneNo,package.OriginalData.ToHexString());
